...
if __name__ == "__main__":
    ...
    objective_index = find_objective_index(model) # Determine the index of the optimisation target defined in the cobra model
    
    max_efms = 1000                                     # Choose how many EFMs are sampled
    
    sample = sample_efms(model,objective_index, max_efms = max_efms)
    full_set = model.get_efms_efmtool()
    all_biomass_efms = full_set[abs(full_set[:, objective_index]) > 1e-7]
    from pyefmsampler.plots import umap_efms
    umap_efms(sample,neighbors = 100)